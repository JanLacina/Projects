...
using SAutoDataScrap.Repository;
...
namespace SAutoDataScrap.Services
{
    public class CarService : ICarService
    {
        ...
        private IList<Car>? ScrapAndProcessData()
        {
            try
            {
        ...
        /// <exception cref="NullReferenceException"></exception>
        private bool SaveCars(IEnumerable<Car> collectionToSave)
        {
        ...
        public bool ScrapNewCars()
        {
            var data = ScrapAndProcessData();
            bool savedProperly = false;

            if(data != null)
            {
                savedProperly = SaveCars(data);
            }
        ...
        public void DeleteDuplicities()
        {
            try
            {
                repository.DeleteDuplicities();
            }
            catch(Exception e)
            {
                logger.LogError("Was not possible to delete duplicities");
            }
        }
